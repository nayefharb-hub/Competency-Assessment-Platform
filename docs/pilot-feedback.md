...
### N27 — the assessment analysis screen, and what the review pass found in it

Not owner feedback: this is the record for the pace work (D28), and for four
defects a review pass found in it that testing had not.

**What shipped.** `/analysis` shows how an assessment was filled in — median
time per control, whether the PM sped up, answers given faster than their own
text can be read, the spread of levels used, and how often evidence was
written. The assessor sees anyone; a PM sees their own and only their own.

**Found by review, fixed here.** Eleven findings across two adversarial passes,
on code whose 235 e2e checks and 47 unit tests were already green — which is
the argument for the gate. The order is severity, not discovery.

1. **The route loaded the unredacted framework on a PM's render.** Every other
   assessee-facing screen calls `getAssesseeFramework()`; this one called
   `getFramework()` for both branches, putting every `target_level` and
   `kib_note` in scope. Nothing rendered them — which is exactly the condition
   the earlier pass called disqualifying on `/`, because the e2e guard checks
   rendered output and would have stayed green through the leak. Now the reader
   is chosen by role.
2. **The client's timing was unfalsifiable.** `dwell_ms` is a number the
   browser sends, so a script could POST a three-minute dwell on all 132
   controls in about a second — and the design's argument for recording pace at
   all was that padding costs the padder their whole afternoon. The server now
   clamps a claimed dwell to the time since the assessment's `started_at`,
   which costs no round trip and restores the argument. It cannot detect a
   plausible lie; nothing can, which is why the screen never shows pace alone.
3. **The stale-schema fallback could keep an old timing on a new answer.**
   PostgREST reports "no such column" two ways, and one of them (`PGRST204`)
   actually means "the column exists, my cache is behind". Dropping the column
   on that code left the previous answer's `dwell_ms` attached to the new one.
   It now retries in full first, and only the genuine 42703 drops the column.
4. **Two figures counted the inactive control and four did not**, so the screen
   could print a `measured` larger than its own `scored`. One population now
   feeds all six.
5. **A revision overwrote the original timing with the revisit's.** Read a
   200-word control for 95 seconds and answer it; come back two days later and
   change your mind in four; the 95 became 4, and that control was then listed
   under "answered faster than the text can be read". A false accusation
   generated by the most ordinary thing a PM does on this screen — and exactly
   the failure D28 rejected the timestamp method to avoid, reintroduced on the
   column itself. `dwell_ms` now means *time to the first answer* and is never
   overwritten; the outbox carries the first reading through a re-commit, and
   the server omits the column rather than nulling it.
6. **`updated_at` cannot order the readings.** The `score_touch` trigger
   rewrites it on every UPDATE, and Submit upserts all 132 score rows to
   prefill the assessor's sheet — so pressing Submit stamps every row with one
   timestamp and the "getting faster?" trend starts splitting an arbitrary heap
   order in half. It could report a PM who sped up as having slowed down, in
   the exact state an assessor looks at. `setAssessorLevels` is more pointed
   still: it re-stamps only the controls the assessor already doubted. Fixed
   with `answered_at`, written by one function and only beside `dwell_ms`.
7. **A clock that went backwards read 0 rather than "unknown".** `Math.max(0,
   …)` made the discard branch dead code, so an NTP step or a laptop resuming
   from suspend produced a 0ms reading — below every control's reading floor,
   i.e. the most severe possible finding about a person, from a hardware event.
   The unit test asserted the 0 and so enshrined it. Now `performance.now()`
   (monotonic) with a latching broken flag, and the test asserts null.
8. **The clock started at hydration, not at paint**, so every full page load
   discarded the reading time before the bundle arrived — an error that runs in
   the accusing direction. Started in the render body now, and the screen's
   wording was softened to what a lower bound can actually support.
9. **A staff `?a=` naming nothing returned a 500** with a database message in
   it. Now an empty card.
10. **`?a=x&a=y` arrives as an array**, and the cast to string sent it into a
    uuid parse error. Handled.
11. **The degraded path degraded too far.** When the pace columns are absent
    the reader returned nothing at all, so a 132-answer assessment reported
    "nothing scored yet" — and threw away the two signals that need no timing
    and matter most. It now drops only the timings.

**Answered, not fixed:** the assessor's picker calls `listAssessments()`, which
pulls every score in the cycle to render a list of names. Real, staff-only, and
pre-existing — `/review`'s overview does the same. Changing a reader shared with
the review path costs more than it saves today; worth doing when either screen
next needs work.
