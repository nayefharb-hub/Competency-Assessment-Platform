...
| 2026-08-13 | `/results` gains a **Strengths & gaps** view (`?view=gaps`) beside By area; the tiles + radar + narrative are a **shared frame**, the toggle sits below the narrative and swaps only the body; narrative leads with a **bold area name** at a ~62ch measure | The same rollup read for action ("what do I develop next") rather than by area. Two symmetric columns of competency blocks (owner chose the detailed form over a compact strengths list, from two prototypes): a gap block opens to its below-target controls weakest-first with the escalation ⚑, a strength block shows its controls strongest-first. The area picture (tiles/radar) and its interpretation (narrative) stay on screen in both views because the narrative *reads the radar*, so a view switch must not take it away. **Control bars in this view are coloured by health** (not exceptions-only like the by-area drill-down): each column is pre-filtered to one kind — gaps are all below-target, strengths all at/above — so every bar in a block is the same story and colour reinforces it rather than adding noise. Exceptions-only is for the drill-down's *mixed* list where on-target rows must recede; the filtering does that job here. |
| 2026-08-13 | Each competency (**L2**) shows its **domain (L1) name** in a single **neutral** chip — **never colour-coded** | Owner wanted the parent domain visible on each competency in the strengths & gaps view (which is ordered by severity, not grouped by area). Colour was the obvious idea and was **rejected on computed evidence, not taste**: a categorical domain palette must stay clear of the reserved health-status hues (Above indigo / Role Ready green / Minor amber / Deficit red) and the azure accent, and the dataviz validator FAILED every candidate trio — magenta collides with Role-Ready green under CVD (ΔE 3.0), violet with the accent (1.9), and so on. **No status-safe categorical domain palette exists in this system.** So domain identity is carried by the chip's **text**, which is accessible by construction and generalises to any framework's domains at any count. The domain name comes from framework **data** (`CeResult.area`), one style for all — no per-domain constant. **Deferred, tenant-owned:** if per-domain colour is ever wanted it becomes a colour *field on the domain*, set and validated in framework-authoring by the tenant; the app cannot ship a safe default palette, so it must not pretend to. |
